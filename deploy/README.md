...
Интерфейс обновляется сам: приложение грузит его с сервера, и после
выкладки открытые экраны переходят на новую сборку (`backend/src/update.rs`).
Переустанавливать APK ради изменений в интерфейсе не нужно.

Новый APK нужен, только если менялся код в `android/`. Тогда:

```bash
cd android
gradle :app:publishApk -PmkVersionCode=2   # номер больше прежнего!
cd .. && ./deploy/deploy.sh                # заберёт dist/android/meshkeeper-2.apk
```

Сервер отдаёт APK с наибольшим номером из `/root/meshkeeper/apk/`
(`/api/app/android`). Приложение проверяет его при запуске и при возвращении
на экран (не чаще раза в час), скачивает в фоне и предлагает установить.
Первое обновление человек подтверждает и один раз разрешает приложению
«установку из этого источника»; на Android 12+ следующие ставятся без
вопросов.

Важно: обновление встанет только поверх APK с **той же подписью**. Поэтому
раздавать надо релизные сборки с одним и тем же keystore. Debug-APK с другой
машины подписан другим ключом — система его отвергнет, и человеку придётся
удалить приложение и поставить заново (с потерей входа).
